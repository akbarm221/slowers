...
@section('title', 'Tambah Data APBD')

@section('content')
    <div class="max-w-4xl mx-auto bg-white dark:bg-gray-800 p-8 rounded-xl shadow-lg">
        <h1 class="text-2xl font-bold text-center mb-2">Tambah Data APBD Baru</h1>
        <p class="text-center text-gray-500 dark:text-gray-400 mb-6">Masukkan tahun dan rincian pendapatan serta belanja.</p>
        <div id="form-pesan" class="hidden p-4 mb-4 text-sm rounded-lg" role="alert"></div>

        <form id="apbd-form">
            {{-- Input Tahun --}}
            <div>
                <label for="tahun" class="block text-sm font-medium">Tahun Anggaran</label>
                <input type="number" name="tahun" id="tahun" class="mt-1 block w-full py-2 px-3 border rounded-md" placeholder="Contoh: {{ date('Y') + 1 }}">
            </div>
            ...
            <div class="mt-8 pt-5 border-t flex justify-end gap-x-4">
                ...
                <button type="submit" id="simpan-apbd" class="btn-primary">Simpan Data</button>
            </div>
        </form>
    </div>
    @push('scripts')
...
<style>
    .input-field {
        @apply mt-1 block w-full py-2 px-3 border border-gray-300 bg-white dark:bg-gray-700 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500;
    }
    .btn-primary {
        @apply px-6 py-2 bg-primary-600 text-white font-semibold rounded-lg shadow-md hover:bg-primary-700;
    }
    .btn-secondary {
        @apply px-4 py-2 bg-gray-200 text-gray-800 dark:bg-gray-600 dark:text-gray-200 font-medium rounded-lg hover:bg-gray-300;
    }
</style>
@endpush